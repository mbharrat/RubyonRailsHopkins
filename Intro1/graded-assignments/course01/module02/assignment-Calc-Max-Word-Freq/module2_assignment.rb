...
#* calculate_word_frequency() - calculates result
  def calculate_word_frequency(content)
    word_f = Hash.new(0)
    content.split.each { |word| word_f[word.downcase] += 1 }
    @highest_wf_count = word_f.values.max
    @highest_wf_words = word_f.select {|k,v| v == @highest_wf_count }.keys
  ...
  def calculate_line_with_highest_frequency()
    ...
    @analyzers.each do |word|
      if word.highest_wf_count > @highest_count_across_lines
        @highest_count_across_lines = word.highest_wf_count
      end 
    end
    @analyzers.select do |word|
      if word.highest_wf_count == @highest_count_across_lines
